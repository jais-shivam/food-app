    ...
    <div>
      <div className="filter-btn-container flex">
        <div className="search m-4 p-4">
          ...
          />
          <button
            className="px-4 py-1 rounded-md bg-green-100 m-4"
            onClick={() => {
        ...
        <div className="m-4 p-4">
          ...
          <input
            type="text"
            value={loggedInUser}
            className="border border-black py-1 my-4 rounded"
      ...
      <div className="res-container flex  flex-wrap justify-start m-4">
        {resData.map((res) => (
          <Link key={res?.info?.id} to={`/restaurant/${res?.info?.id}`}>
            {res?.info?.totalRatingsString === "1K+" ? (
              <RestaurantCardWithReviewLabel data={res?.info} />
            ) : (
